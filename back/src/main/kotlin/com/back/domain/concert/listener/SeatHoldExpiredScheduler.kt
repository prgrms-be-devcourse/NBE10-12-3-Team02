...
// Score <= 현재 시각인 항목을 만료된 좌석으로 판단함 (1회당 BATCH_SIZE=500건으로 메모리/레디스 보호)
...
class SeatHoldExpiredScheduler(
    ...
    fun processExpiredSeats() {
        ...
        // Score <= 현재 시각인 항목 최대 BATCH_SIZE(500건) 조회
        val expiredMembers = stringRedisTemplate.opsForZSet()
            .rangeByScore(SeatOccupyManager.EXPIRE_QUEUE_KEY, 0.0, now, 0, BATCH_SIZE)
